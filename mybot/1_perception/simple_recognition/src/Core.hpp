...
void findBlob(cv::Mat& binary, cv::Mat& idMat, std::vector<Blob>& blobs, int minIndex=0) {
  // binary라 함은, object 영역은 255로, background 영역은 0으로 표현한 image
  ...
  for (auto i=0; i<idMat.rows; i++) {
    for (auto j=0; j<idMat.cols; j++) {
      ...
      if (tempBlob.numPixel<minIndex || tempBlob.numPixel>32000 || tempBlob.rect.width>120) {
        idMat.setTo(0, mask);
        continue;
      }
...
bool findMaxBlob(cv::Mat& binary, cv::Mat& idMat, Blob& blob, int minIndex=0) {
	// binary라 함은, object 영역은 255로, background 영역은 0으로 표현한 image
...
void findSamplePoints(cv::Mat& mask, Eigen::MatrixXf& points, Eigen::MatrixXf& sampledPoints) {
        cv::Mat idx;
        std::vector<cv::Mat> splited;
...
//!!!!
void countSphereMask(Eigen::Vector3f& center, Eigen::MatrixXf& points, Eigen::Vector3f& handCenter, int maxCount, float thDistance=50.) {
      int count=0; 

      Eigen::MatrixXf handPoints(maxCount, 3);
      ...
      for (int i=0; i<points.rows(); i++) {
          if (distanceXf(i)<thDistance) {
              handPoints.row(count++) = points.row(i); 
          }
      }

      handCenter = handPoints.colwise().mean(); 
}
...
void createViewer(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, std::string viewerName) {
    pointCloudViewer = boost::shared_ptr<pcl::visualization::PCLVisualizer> (new pcl::visualization::PCLVisualizer (viewerName)); 
}

void setViewer(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, cv::Point pt, cv::Size size, cv::Scalar color = cv::Scalar(0,0,0)) {
  pointCloudViewer->initCameraParameters(); 
  pointCloudViewer->setBackgroundColor(color.val[2]/255., color.val[1]/255., color.val[0]/255.); 
  std::cout << pt << std::endl; 
  pointCloudViewer->setPosition(pt.y, pt.x); 
  pointCloudViewer->setSize(size.height, size.width); 
}
...
void applyPointCloud(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr pointCloudPtr) {
...
void getSphereCloud(std::vector<pcl::ModelCoefficients>& pclModels, Eigen::Vector3f centerPos3d) {
    pcl::ModelCoefficients model; 
    model.values.resize(4); 
    model.values[0] = centerPos3d(0)/1000.;
    model.values[1] =-centerPos3d(1)/1000.;
    model.values[2] = centerPos3d(2)/1000.;
    model.values[3] = 0.1; 
    pclModels.push_back(model);    
}

void updateSphereCloud(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, int code, Eigen::Vector3f centerPos3d, cv::Scalar color = cv::Scalar(0,0,0)) {
        ...
        pointCloudViewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 5, name);
...
void applySphereCloud(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, std::vector<pcl::ModelCoefficients>& pclModels, std::vector<cv::Scalar>& colors) {
  for (int i=0; i<(int)pclModels.size(); i++) {
          std::string name = std::string("sphere")+std::to_string(i); 
          pointCloudViewer->addSphere(pclModels[i], name); 
          pointCloudViewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_REPRESENTATION, 1, name);
          pointCloudViewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, colors[i].val[2], colors[i].val[1], colors[i].val[0], name);
          pointCloudViewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, 0.2, name);
  }
}


void updateLineCloud(boost::shared_ptr<pcl::visualization::PCLVisualizer>& pointCloudViewer, Eigen::Vector3f& point1, Eigen::Vector3f& point2, cv::Scalar color, int idx=0) {
    ...
    pointCloudViewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_LINE_WIDTH, 8, std::string("cylinder")+std::to_string(idx));
...
/*
bool getPointCloud(cv::Mat& Depth_mat, Eigen::MatrixXf& points, pcl::PointCloud<pcl::PointXYZ>::Ptr pointCloudPtr, Eigen::VectorXf& plane) {

  int gridSpace;
  // 유효한 인덱스만 골라내보자.
  double ec=20.;
  double bestInlierRatio=0.3;
  cv::Mat idx;
  cv::Mat mask = cv::Mat::zeros(Depth_mat.size(), CV_8UC1);

  cv::Mat maskElement = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(125,5));
  cv::Mat depthFilterMat = Depth_mat>1000;
  cv::dilate(depthFilterMat, depthFilterMat, maskElement);
  mask.setTo(0, (Depth_mat==0)|depthFilterMat);
  mask.setTo(1, (Depth_mat!=0));
  if (cv::countNonZero(mask)==0) return true; 
  cv::findNonZero(mask, idx);
  Eigen::MatrixXf distance;

  if (idx.rows<3333)
    gridSpace=1;
  else gridSpace = idx.rows/3333;
  ...
  for (auto i=0, si=0; i<idx2.rows; i+=gridSpace, si++) {
    selectedIdx.push_back(idx2.at<int>(i,0));
    sampledPoints.row(si) = points.row(idx2.at<int>(i,0));
  }
  ...
  distance = ((sampledPoints*plane).array()-1).array()/plane_norm;
  for (auto i=0; i<sampledNumData; i++) {
    if (fabs(distance(i))<ec) inlier_idx.push_back(i);
  }

  if (inlier_idx.size()/(float)sampledNumData>=bestInlierRatio) {
    // Inlier index
    Eigen::MatrixXf pts_inlier(inlier_idx.size(),3);
    for (std::size_t i=0; i<inlier_idx.size(); i++) {
      pts_inlier.row(i) = sampledPoints.row(inlier_idx[i]);
    }
  ...
  for (std::size_t i=0; i<selectedIdx.size(); i++) {
    point.x = points.row(selectedIdx[i])[0];
    point.y = points.row(selectedIdx[i])[1];
    point.z = points.row(selectedIdx[i])[2];
    pointCloudPtr->points.push_back(point);
  }
...
/*
void planeParameter_pcl(cv::Mat& imgDepth, Eigen::MatrixXf& points, Eigen::VectorXf& parameter) {

  pcl::PointCloud<pcl::PointXYZ>::Ptr pointCloudPtr_depth (new pcl::PointCloud<pcl::PointXYZ>); // for rgb scale.
  bool pass = getPointCloud(imgDepth, points, pointCloudPtr_depth, parameter);
  if (pass==true) return;

  static pcl::ModelCoefficients coefficients;
  ...
  seg.setDistanceThreshold(20.0);
  seg.setMaxIterations(999);
  seg.setProbability(0.3);
  seg.setInputCloud(pointCloudPtr_depth);
  ...
  parameter[0] = coefficients.values[0]/d;
  parameter[1] = coefficients.values[1]/d;
  parameter[2] = coefficients.values[2]/d;

  pointCloudPtr_depth.reset();
  inliers.reset();
}
*/
...
void project(cv::Mat& src , cv::Mat& dst, Eigen::MatrixXf points, Eigen::VectorXf& parameter, Eigen::MatrixXf& K, Eigen::MatrixXf& prPoints) {
  ...
  Eigen::MatrixXf distM = (prPoints*parameter).array()-1;
  distM = distM.array()/parameter.norm();
  prPoints-=(distM*parameter.transpose()/parameter.norm());
//  prPoints = prPoints.cwiseQuotient(prPoints.col(2).replicate(1,3));

  Eigen::MatrixXf prIdx = prPoints*K.transpose();
