...
int test(double fx, double fy, double cx, double cy, ros::Publisher& point_pub, ros::Publisher& pub_psw, bool saveMode=false){
        ...
        static pcl::PointCloud<pcl::PointXYZRGB>::Ptr RGBD_Cloud, recognition_Cloud, symbolic_Cloud, geometric_Cloud, thermal_Cloud;
        static pcl::PointCloud<pcl::PointXYZRGB>::Ptr geo1_Cloud, geo2_Cloud; 

        static boost::shared_ptr<pcl::visualization::PCLVisualizer> RGBD_Viewer, recognition_Viewer, symbolic_Viewer, geometric_Viewer, thermal_Viewer; 
        ...
        boost::call_once([]
                        ...
                        // PCL1-1) Create viewer
                                ...
                                // PCL1-2) Set viewer
                                plaincode::setViewer(RGBD_Viewer, cv::Point(100,300), cv::Size(300,500), cv::Scalar(255,255,255)); 
                                plaincode::setViewer(recognition_Viewer, cv::Point(400,300), cv::Size(300,500), cv::Scalar(255,255,255)); 
                                plaincode::setViewer(symbolic_Viewer, cv::Point(700,300), cv::Size(300,500), cv::Scalar(255,255,255)); 
                                plaincode::setViewer(geometric_Viewer, cv::Point(100,600), cv::Size(300,500), cv::Scalar(255,255,255)); 

                                plaincode::setViewer(geo1_Viewer, cv::Point(100,400), cv::Size(300,500), cv::Scalar(255,255,255)); 
                                plaincode::setViewer(geo2_Viewer, cv::Point(700,400), cv::Size(300,500), cv::Scalar(255,255,255)); 

                                if (thermal_mode) plaincode::setViewer(thermal_Viewer, cv::Point(700,600), cv::Size(300,500), cv::Scalar(255,255,255)); 
        ...
        if (thermal_mode && !originThermal.empty()) {
                ...
                Thermal.setTo((unsigned short)min, Thermal<(unsigned short)7960);
        }
